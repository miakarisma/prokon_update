...
<head>
  ...
  <!-- custom css -->
  <link rel="stylesheet" href="/css/category-edit.css">
...
<body>
  ...
  <?= \Config\Services::validation()->listErrors();?>
        ...
        <form action="/category/update/<?= $category['id']; ?>" method="POST" enctype="multipart/form-data">
            <?= csrf_field() ?>
            <input type="hidden" name="imagePrevName" value="<?= $category['image'] ?>">

            <label for="image" id="image" class="img-label">Category Image</label>
            <img src="/img/<?= $category['image'] ?>" width="200px" class="img-preview">
